...
为了满足“不使用 PyTorch / TensorFlow / JAX 自动微分”的要求，本实验自行实现了一个轻量级计算图系统。每个张量对象都保存：
...
在前向传播阶段，程序构建计算图；在调用 `backward()` 时，按照拓扑排序逆序执行各节点的梯度回传。当前实现支持：
...
交叉熵损失由 `log_softmax + negative log-likelihood` 组成，从而可以稳定地完成多分类训练。
...
训练脚本会输出训练集与验证集的 `Loss` 曲线，对应文件为：

- `outputs/final_run/loss_curve.png`
...
- `outputs/final_run/val_accuracy_curve.png`

验证集准确率从 `0.8527` 持续提升，在第 13 个 epoch 达到最佳值 `0.8998`。这一结果说明：

- 学习率衰减策略有效提升了训练稳定性
- 两层隐藏层的三层 MLP 对 Fashion-MNIST 已具备较强表示能力
- 在训练后期，训练集准确率继续上升，但验证集提升趋于放缓，说明模型开始接近过拟合边界
...
- `outputs/final_eval/confusion_matrix.png`

从混淆矩阵可以观察到以下现象：

- `Trouser`、`Bag`、`Ankle boot` 的识别效果较好，因为这些类别的轮廓较为鲜明，和其他类别差异较大。
- `Shirt` 是最难识别的类别之一，经常被分成 `T-shirt/top`、`Pullover`、`Coat`，说明这些上衣类服装在灰度低分辨率图像下具有较强相似性。
- `Coat` 和 `Pullover` 之间也存在明显混淆，这与二者在整体轮廓和袖部形态上的相似性有关。
- 鞋类之间仍有少量混淆，例如 `Ankle boot` 有一部分被预测为 `Sneaker`，`Sandal` 也会与其他鞋类产生混淆。

## 8. 第一层权重可视化分析

第一层权重可视化文件为：

- `outputs/final_run/first_layer_weights.png`

将第一层的权重矩阵从 `784` 维重新恢复成 `28 x 28` 图像后，可以发现若干典型模式：

- 一部分权重表现为明显的边缘检测器，对水平边缘、竖直边缘或斜向轮廓响应较强。
- 一部分权重更关注图像中央区域，说明网络学到了服装主体的整体位置分布。
- 还有一些权重在上半区域或下半区域有更强响应，可能对应上衣领口、袖子，或者鞋底、裤腿等局部结构。
...
- `outputs/final_eval/misclassified_samples.png`

对错分样本进行观察后，可以从以下角度理解错误原因：

- `Shirt` 和 `T-shirt/top` 的边界并不总是清晰，尤其在领口和袖口区域特征不明显时，模型很难准确区分。
- `Coat`、`Pullover`、`Shirt` 等类别都属于上衣类，整体外形相近，容易在低分辨率下混淆。
- 某些样本本身轮廓不完整，或者衣物与背景对比度较低，会影响模型对关键区域的判断。
- 本实验模型是纯全连接网络，没有显式利用局部卷积结构，因此对于局部纹理和位移不变性建模较弱，这也是产生错分的重要原因。
